...
def open_selected(mes):#This is page
    try:
        ...
        # Create The Treeview
        my_tree = ttk.Treeview(tree_frame, yscrollcommand=tree_scroll.set, selectmode="extended", height=25) #25
        my_tree.pack()
        ...
        my_tree.column("data", anchor=W, width=60)
        my_tree.column("prazo", anchor=CENTER, width=60)
        my_tree.column("valor", anchor=CENTER, width=75)
        my_tree.column("cob", anchor=CENTER, width=30)
        my_tree.column("pago", anchor=CENTER, width=30)
        my_tree.column("obs", anchor=E, width=500)
        ...
        data_frame.place(x=10,y=550,height=150,width=1000)
        ...
        n_entry.grid(row=0, column=1, ipadx=50)
        ...
        val_entry.grid(row=1, column=1, pady=2, ipadx=50)
        ...
        obs_entry.grid(row=0, column=7, columnspan=5, ipadx=100)

        cob_btn = Button(data_frame, text="Cobrar dia", command=lambda: cob_dia(mes), anchor= CENTER)
        cob_btn.grid(row=1, column=7)

        cobsel_btn = Button(data_frame, text="Cobrar selec.", command=lambda: cob_selected(mes), anchor= CENTER)
        cobsel_btn.grid(row=1, column=8)

        acd_list_btn = Button(data_frame, text="Posç. selec.", command=lambda:cob_posicao(mes),anchor= CENTER)
        acd_list_btn.grid(row=1, column=9, pady=5) 

        clear_btn = Button(data_frame, text="Limpar inf", command=lambda:clear_entries(), anchor= CENTER)
        clear_btn.grid(row=1, column=10, pady=5)

        update_btn = Button(data_frame, text="Aplicar", command=lambda: update_record(mes),anchor= CENTER)
        update_btn.grid(row=2, column=10, pady=5)

        add_acd_btn = Button(data_frame, text="Add acd", command=lambda:add_acordo(mes),anchor= CENTER)
        add_acd_btn.grid(row=2, column=1, pady=5) 

        canc_btn = Button(data_frame, text="Cancelar acd", command=lambda: del_and_sort(mes),anchor= CENTER)
        canc_btn.grid(row=2, column=3)

        delNO_btn = Button(data_frame, text="Del acd", command=lambda:del_no_sort(mes),anchor= CENTER)
        delNO_btn.grid(row=2, column=5, pady=5) 

        del_btn = Button(data_frame, text="Del agenda", command=lambda:drop_table(mes),anchor= CENTER)
        del_btn.grid(row=2, column=7, pady=5) 

        acd_list_btn = Button(data_frame, text=f"Exl {str(mes)}", command=lambda:get_excel(mes),anchor= CENTER)
        acd_list_btn.grid(row=2, column=8, pady=5)

        acd_list_btn = Button(data_frame, text="Lista desf.", command=lambda:open_list(mes),anchor= CENTER)
